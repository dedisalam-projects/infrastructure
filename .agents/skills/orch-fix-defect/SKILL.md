...
description: "Use when existing behavior is broken or wrong. — Orchestrate fixing a bug — reproduce it as a failing regression test, fix to green, review, and gated commit — by delegating each phase to the matching ECC agent"
metadata:
  origin: ECC
---
...
> [!IMPORTANT]
> **Rule Adherence**: Always strictly follow the `orch-fix-defect` conventions outlined above to ensure workspace consistency and prevent regressions.
