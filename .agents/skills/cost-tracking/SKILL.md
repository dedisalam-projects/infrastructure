---
name: cost-tracking
description: "Use when the user asks about costs, spending, usage, tokens, budgets, or cost breakdowns by model, session, or date. — Track and report Claude Code token usage, spending, and budgets from the local ECC cost-tracker metrics log"
metadata:
  origin: community
---
...
> [!IMPORTANT]
> **Rule Adherence**: Always strictly follow the `cost-tracking` conventions outlined above to ensure workspace consistency and prevent regressions.
