...
- **Failing criterion**: `AC-001: The export works correctly and is secure.`
  - *Reason*: Vague terms ("correctly", "secure") are unobservable; lacks scenario, action, expected output, and verification method.
- **Passing criterion**:
  ```markdown
  AC-001: Export generates file with correct headers
  - Scenario: authenticated user, at least one data row visible
  ...
  - Expected: browser downloads file with columns [id, name, created_at]
  - Must not: expose internal fields or rows belonging to other users
  - Verification: automated integration test + schema check
  - Priority: Required
  ```
- **Failing context**: `Discovered facts: Users on free tier limited to 100 exports/mo.`
  - *Reason*: Business limits belong under explicit Product constraints or assumptions, never inferred silently from code.

### Pass/Fail Rubric

A brief passes only if all checks are true:
- [ ] Every required criterion has a scenario, observable outcome, and verification method
- [ ] Vague terms replaced with observable evidence or marked as human judgment
- [ ] Product/business constraints listed as supplied/assumed (not inferred from code)
- [ ] Scope and out-of-scope boundaries are explicitly named
- [ ] Blocking decisions limited to choices affecting safety or correctness
