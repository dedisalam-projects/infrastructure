...
description: "Use when working on the tinystruct codebase or any project built on tinystruct — including creating Application classes, @Action-mapped routes, unit tests, ActionRegistry, HTTP/CLI dual-mode handling, the built-in HTTP server, the event system, JSON with Builder/Builders, database persistence with AbstractData, POJO generation, Server-Sent Events (SSE), file uploads, and outbound HTTP networking. — Expert guidance for developing with the tinystruct Java framework"
metadata:
  origin: ECC
---
...
> [!IMPORTANT]
> **Rule Adherence**: Always strictly follow the `tinystruct-patterns` conventions outlined above to ensure workspace consistency and prevent regressions.
