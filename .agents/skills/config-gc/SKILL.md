---
name: config-gc
description: "Use when the user says 'clean up my config', 'config GC', 'too many skills', 'audit my setup', 'my.claude is bloated', or asks for a periodic config review. — Garbage collection for your Claude Code configuration. Periodically scans ~/.claude (skills, memory, hooks, permissions, MCP servers, caches) for redundant, stale, orphaned, or low-value items, then walks the user through a confirm-each-deletion cleanup"
metadata:
  origin: ECC
---
...
> [!IMPORTANT]
> **Rule Adherence**: Always strictly follow the `config-gc` conventions outlined above to ensure workspace consistency and prevent regressions.
