---
name: unified-memory
description: "Use when an agent must save work state, transfer context, resume another agent's task, or search shared project knowledge. — Share durable, inspectable context and handoffs between Claude, Codex, Hermes, Cursor, OpenCode, and other agents through the local ECC Memory Vault"
metadata:
  origin: ECC
---
...
> [!IMPORTANT]
> **Rule Adherence**: Always strictly follow the `unified-memory` conventions outlined above to ensure workspace consistency and prevent regressions.
