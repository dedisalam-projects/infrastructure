---
name: agent-sort
description: "Use when ECC should be trimmed to what a project actually needs instead of loading the full bundle. — Build an evidence-backed ECC install plan for a specific repo by sorting skills, commands, rules, hooks, and extras into DAILY vs LIBRARY buckets using parallel repo-aware review passes"
metadata:
  origin: ECC
---
...
> [!IMPORTANT]
> **Rule Adherence**: Always strictly follow the `agent-sort` conventions outlined above to ensure workspace consistency and prevent regressions.
